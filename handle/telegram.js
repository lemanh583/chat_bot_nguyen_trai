...
const questionsJson = require("../test-bot.questions.json")
...
class TelegramBot {
  ...
  listen() {
    ...
    this.bot.start(async (ctx) => {
      ...
      const question = questionsJson.find((item) => item?.command === '/start')
      const options = questionsJson.filter((item) => item?.parent_id?.$oid === question._id?.$oid)
      const buttons = options.map((item) => {
        return {
          text: item.option,
          callback_data: "call_" + item._id.$oid.toString(),
        };
      });
    ...
    this.bot.action(/^call_.*/, async (ctx) => {
      ...
      const replyText = questionsJson.find((item) => item._id?.$oid === data?.data.replace("call_", ""))
      if (!replyText) {
        ctx.reply("Not Found");
        return;
      }
      // const options = await questionSchema.find({ parent_id: replyText._id }).sort({ createdAt: 1 });
      const options = questionsJson.filter((item) => item.parent_id?.$oid === replyText._id?.$oid)
      const buttons = options.map((item) => {
        return {
          text: item.option,
          callback_data: "call_" + item._id.$oid.toString(),
